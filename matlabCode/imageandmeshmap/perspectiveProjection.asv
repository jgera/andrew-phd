...
% direction of robot
    ...
    tr1=fkine(a.r,image_dat(curr_im).robot_pose); %Forward kinematics for joint 6 location
    ...
    tr = tr1*M*N; %Transform of the camera location on the robot arm    
    endefectr= tr(1:3,4)';   % the point of the end effector      
    bear=tr(1:3,3)'; %straight    
...
%     line([tr(1,4),tr(1,3)+tr(1,4)],[tr(2,4),tr(2,3)+tr(2,4)],[tr(3,4),tr(3,3)+tr(3,4)],'Color','g'); %bear (straight) ahead line
    line([endefectr(1),tilt_rotate_vec(1)+endefectr(1)],[endefectr(2),tilt_rotate_vec(2)+endefectr(2)],[endefectr(3),tilt_rotate_vec(3)+endefectr(3)],'Color','r');
    ...
    %Lens field of view is 43'36 x 33'24 = 43.6 x 33.4 for a 1/3' sensor
...
%     hold on;      
    ...
    tcolor = [.2 .7 .2];
    patch(x2,y2,z2,tcolor);
    plot3(vertsOnImagePlane(:,1),vertsOnImagePlane(:,2),vertsOnImagePlane(:,3)); %Show the 2D projection points 
    axis equal
