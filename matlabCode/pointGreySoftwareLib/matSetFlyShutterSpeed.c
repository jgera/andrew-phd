 ...
 * Set shutter speed
 */
int SetCamShutterSpeed(double dContext, double dValueA, double dValueB)
{
    ...
    {
        printf( "\nflycaSetCameraProperty(ShutterSpeed): %s\n", flycaptureErrorToString( error ) );
        return 0;
    }
              ...
              
{
  ...
  /* Call the bridging function to set camera shutter speed */
  if( !SetCamShutterSpeed(*dContext,*dValueA,*dValueB) )
  {
     mexErrMsgTxt( "matSetFlyShutterSpeed failed !!" );
  }
}

/*
 * To Compile use the following command
 * mex -g -f "pgrmex.bat" matSetFlyShutterSpeed.c
 */